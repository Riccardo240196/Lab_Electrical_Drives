...
v = @(t) interp1(0:0.2:1.2,[0 0.75 0 0 -0.75 0 0],t);
time = linspace(0,1.2,10000);    
a = diff(v(time))./diff(time);
tACC = 0.2;
figure
subplot(2,1,1);
plot(time,v(time),'linewidth',2); grid on
title('Triangular speed profile','interpreter', 'latex');
...
k = omega_max/v_max;            % Gear ratio
Jmass = m/(k^2);                % Mass M.O.I
Jmot = 0.2*(Jtrans+Jmass);      % Assumption
Jm = 1e-4;                      % Motor M.O.I
Jmot = Jm;
